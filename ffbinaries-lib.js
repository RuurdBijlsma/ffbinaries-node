...
function downloadUrls(components, urls, opts, callback) {
  ...
  if (typeof urls === 'object') {
    remappedUrls = _.map(urls, function (v, k) {
      return (!components || components && !Array.isArray(components) || components && Array.isArray(components) && components.indexOf(k) !== -1) ? {
        component: k,
        url: v
      } : null;
    });
      ...
      try {
        ...
        const request = http.get(url, (response) => {
          ...
          if (response.statusCode === 200) {
            ...
            const file = fs.createWriteStream(cacheFileTempName);
            // Pipe the response into the file stream
            response.pipe(file);
            response.on('data', (chunk) => {
              runningTotal += chunk.length;
            });
            // Log a message when the file is saved
            file.on('finish', () => {
              file.close();
